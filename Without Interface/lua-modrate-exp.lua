local m_config = {
    ...
    --[[
     ...
     --
    --[[ DON'T TOUCH THIS ]] -- counter for display menu
    counter = 0
}
 --

--[[ DON'T TOUCH THIS ]] function m_config.resetCounter()
    m_config.counter = 0
end
 --

--[[ DON'T TOUCH THIS ]] CharDBQuery("CREATE DATABASE IF NOT EXISTS `" .. m_config.elunaDB .. "`;")
CharDBQuery(
    "CREATE TABLE IF NOT EXISTS `" ..
        m_config.elunaDB ..
            "`.`characters_exp_rates` (`guid` int(10) NOT NULL, `mod_exp` INT(2) NOT NULL DEFAULT 1, PRIMARY KEY (`guid`)) ENGINE=InnoDB DEFAULT CHARSET=latin1;"
)

local m_exp = {}
 --

--[[ Load player exp rates after connect ]] function m_exp.onConnect(event, player)
    ...
    local GetRateExp =
        CharDBQuery("SELECT mod_exp FROM " .. m_config.elunaDB .. ".characters_exp_rates WHERE guid = " .. pGuid .. ";")
    if GetRateExp ~= nil then
        m_exp[pGuid].mod_exp = GetRateExp:GetUInt32(0)
    else
        local AddRateExp =
            CharDBQuery(
            "INSERT INTO " .. m_config.elunaDB .. ".characters_exp_rates (guid, mod_exp) VALUES (" .. pGuid .. ", 1);"
        )
...
--[[ Save player exp rates before disconnect ]] function m_exp.onDisconnect(event, player)
    local pGuid = player:GetGUIDLow()
    if not (m_exp[pGuid]) then
        m_exp[pGuid] = {
            mod_exp = 1
    ...
    local SaveRateExp =
        ...
        "UPDATE " ..
            m_config.elunaDB ..
                ".characters_exp_rates SET mod_exp = " .. m_exp[pGuid].mod_exp .. " WHERE guid = " .. pGuid .. ";"
    )
end
RegisterPlayerEvent(4, m_exp.onDisconnect)
 --

--[[ Add amount of experience multiply by Rate ]] function m_exp.onReceiveExp(event, player, amount, victim)
    local pGuid = player:GetGUIDLow()
    if not (m_exp[pGuid]) then
        m_exp[pGuid] = {
            mod_exp = 1
...
--[[ Load All Players Exp Rates After Reloading ]] function m_exp.getAllPlayerExp(event)
...
--[[ Save All Players Exp Rates Before Reloading ]] function m_exp.saveAllPlayerExp(event)
    for i, player in ipairs(GetPlayersInWorld()) do
